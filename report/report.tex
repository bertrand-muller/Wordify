...
Pour cela, nous avons dû utiliser des outils complémentaires et veiller à les associer au framework Laravel. Cette phase a constitué une partie importante de notre travail. En effet, nous avons dû configurer un outil appelé \textit{Redis} qui permet de sauvegarder les événements déclenchés par le serveur en réponse aux interactions faites par un ou plusieurs joueurs. D'autre part, il était nécessaire de configurer \textit{Laravel Echo Serveur}, lui-même à l'écoute de \textit{Redis}, pour transmettre les événements au client (utilisateur final) et changer en conséquence les pages les réceptionnant.
